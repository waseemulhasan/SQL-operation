...
create database if not exists analyst;
use analyst;
...
select * from bank_details where age< 35 and housing = 'yes';
...
select * from bank_details where marital = "married" and age = 35;
select * from bank_details where education = 'unknown' or marital = 'unknown';
...
select balance, housing from bank_details where housing = 'yes';
...
DELIMITER &&
create procedure data2(IN v1 varchar (30), IN v2 varchar(30))
begin 
    select * from bank_details where EDUCATION  = 'v1' and job = 'v2';
end &&
call data1( 'V1', 'V2');
CALL DATA2('SECONDARY','UNKNOWN')
DELIMITER &&
